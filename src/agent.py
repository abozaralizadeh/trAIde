...
def run_trading_agent(
  ...
  instructions = (
    ...
    "## CRITICAL — Trade the strongest setup; code enforces survival:\n"
    "- Your job is to MAKE MONEY, and that means taking trades. Code — not you — enforces survival: every entry is "
    "capped by a hard per-trade risk budget, a per-symbol concentration cap, portfolio-heat and daily-drawdown circuit "
    ...
    "- Let winners run: do not pre-empt a working trend trade; the code trails and protects it. Size UP with conviction "
    "toward the risk-budget cap rather than defaulting to the smallest position.\n"
    "- Do not manufacture confidence, loosen protection, or force a fill on a symbol with no real edge just to create "
    "activity — but a clean edge that clears the risk caps is NOT 'forcing'; take it.\n"
    ...
    "Do NOT spend the entire run on a single symbol, and do not re-take the same losing symbol repeatedly.\n"
    "- EXCEPTION (genuine no-setup tape only): when the majors are gated in BOTH directions and no liquid, established "
    "coin offers any clean edge, standing aside is correct — not a failure. But this is the rare exception, not the "
    "default; do not reach for low-liquidity or freshly-listed names to manufacture a trade.\n\n"
    ...
    "**setup_family (REQUIRED on every place_futures_limit_order call):**\n"
    "- Declare which PLAYBOOK the trade is: 'continuation' (with an established trend), 'fade_extreme' "
    "(fading a stretched move back toward value), 'breakout' (a break of a range/level), or 'range_edge' "
    "(buying support / selling resistance inside a range).\n"
    "- This is NOT bookkeeping. Each family keeps its own measured forward-return score "
    ...
    "confluence, and for a HIGH-CONVICTION continuation (confidence >= the marketable threshold) place a MARKETABLE "
    "entry that crosses slightly to fill immediately. The atomic bracket still attaches, so a marketable fill is fully "
    "protected. TP/SL, daily/regime alignment, and RR must remain valid — but capturing the move beats a perfect "
    "unfilled level.\n"
    ...
    "- If current price is at/through your level: for a low/medium-conviction setup, wait for a pullback limit or rotate. "
    "For a HIGH-CONVICTION trend continuation, place a marketable limit that crosses up to the code's marketable band to "
    "secure the fill — the bracket attaches, so it is protected. Pure market entry tools remain close/emergency-only.\n"
    "- A fresh breakout impulse is best traded on its first confirmed retest/pullback; but if the trend is strong and a "
    "clean pullback is not offered, a bracketed marketable continuation entry is preferable to missing the move entirely.\n"
    "- For LOW/MEDIUM-conviction entries, do not rest the limit worse than current price; wait for the pullback/bounce.\n\n"

    "## OPTIMAL ENTRY PLANNING — decide *where* the highest-EV fill is before you place anything:\n"
    "Getting the direction right is not enough; a great trend entered at the wrong price is a loss. Before every "
    "entry, explicitly reason about the best arrival price, then choose HOW to get there:\n"
    "- Read `entryMap` in the analysis: `extensionAtrLong/Short` is how many 15m ATRs price already sits beyond the "
    "15m VWAP in your direction. Large positive = you are LATE in the leg (price is stretched); near zero / negative = "
    "you are at or before value.\n"
    ...
    "## Performance awareness (small samples are noise, not a verdict):\n"
    "- Check the performanceSummary field, but calibrate to sample size. A few dozen closes CANNOT establish that you "
    "have no edge — treating an early losing streak as proof and shrinking to fee-dust only guarantees you never earn "
    "back the drawdown (the account's own history: over-tightening after small losses froze it into more losses). Code "
    "already shrinks risk on a losing direction/symbol and benches repeat losers; you do NOT also need to stand aside.\n"
    "- If avg loss exceeds avg win by 2x+: the usual cause is cutting winners early and letting losers run to the stop. "
    "The fix is to HOLD winners longer (structural target, trend trailing), not to trade smaller — keep the invalidation "
    "structural and let the code caps control the dollar loss.\n"
    "- If recent PnL is negative: prioritize trade QUALITY and trend alignment (trade the strongest names with room to "
    "target), not smaller size. A correct trade sized to fee-dust cannot dig the account out.\n"
  ...
  research_agent = Agent(
    ...
    instructions=(
      ...
      "- MACRO CALENDAR (you own this — nothing else refreshes it): the Trading Agent opens no NEW risk in "
      "the hour before a high-impact scheduled release, and may trade the hour after it as "
      "setup_family='macro_event'. Both depend entirely on a calendar only you can supply. If "
  ...
  try:
    ...
    user_state_obj["edgeReport"] = {
      ...
      "note": (
        f"SAMPLE SIZE FIRST: every statistic below is computed over rolling.n={_edge_stats_now.get('n', 0)} closed "
        "trades. Under ~15 closes a win rate, payoff or loss streak is mostly noise — a 6-loss run happens by chance "
        "in a coin-flip system. Read a small-n report as WEAK evidence: it should sharpen which setups you take, never "
        "argue you out of trading altogether. Standing aside produces no new closes, so a bleak small sample can only "
        "get staler and bleaker if you let it stop you. "
        ...
        "The TP is a BACKSTOP that caps a runaway move; it is entirely normal and expected for it to sit beyond the "
        "typical excursion. So a low mfeReachedRate at 1R/2R is NOT a reason to stand aside, and it is NOT evidence "
        "your targets are wrong — it is simply how this system exits. Use targetReachability for ONE purpose: to keep "
        "the stop honest. Never pull the stop inside true invalidation to make a ratio work. If a setup has clean "
        "structure, an invalidation level you believe, and clears baseRr from that level, TAKE IT — standing aside is "
        "only correct when the STRUCTURE is absent, never merely because the target is far away. "
