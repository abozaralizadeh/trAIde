...
- **Trailing ratchet** (`PROFIT_LOCK_TRAIL_ENABLED`, default ON): the primary "let winners run" mechanism, which **replaces** the give-back market-close. The give-back was capping *every* winner at ~1.1R gross — it exits the instant price retraces a fixed % of peak, so a normal wobble-then-continue never reaches the 1.5–2R bracket TP, and fees then shrink that 1.1R to ~0.4R net (the account's realized-R data showed every win < 1R while losses ran −0.4 to −0.7R). Instead, once a trade arms (≥ `PROFIT_LOCK_BREAKEVEN_TRIGGER_R`), the **stop ratchets up** to lock `peak − PROFIT_LOCK_TRAIL_DISTANCE_R × own-risk`, never below fee-breakeven and never backwards. Because it's a resting stop (not a market close), the trade rides shallow pullbacks to its TP or trails a genuine runner, while breakeven-lock still guarantees no loss. Self-normalizing in R units — no ATR feed, no percentage to tune. (R is anchored to the lifecycle's *original* risk, so it keeps working after the stop reaches breakeven.) The give-back close below remains available for `PROFIT_LOCK_TRAIL_ENABLED=false`.
...
| `PROFIT_LOCK_TRAIL_DISTANCE_R` | `1.0` | Trail the stop this many R below the running peak once the trade arms |
