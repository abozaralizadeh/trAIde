...
- **Trailing ratchet** (`PROFIT_LOCK_TRAIL_ENABLED`, default ON): the primary "let winners run" mechanism, which **replaces** the give-back market-close. **Jul 23 2026 give-back fix:** the trail originally armed only at +1R and trailed a full 1R behind the peak — but the account's real trades peak on a **0.3–1.7R scale** (16 of 19 recent closes peaked *below* +1R), so the trail never engaged and green trades round-tripped all the way back to a full stop-out (ERA +0.85R→−0.05, ONDO +0.80R→+0.44, RE +1.72R→+0.78, giving back ~1R). Now the trail (a) **arms at `PROFIT_LOCK_TRAIL_ARM_R`** (0.5R — the trade has shown half its own risk as profit, a real edge beyond noise) and (b) **locks the GREATER of** `PROFIT_LOCK_TRAIL_LOCK_FRAC × peak` and `peak − PROFIT_LOCK_TRAIL_DISTANCE_R × own-risk` — the fraction captures the mid-size runs that dominate the data, the fixed-R trail captures genuine big runners. Floored at fee-breakeven, never backwards. Because it's a resting stop (not a market close), the trade rides shallow pullbacks to its TP or trails a runner. Self-normalizing in R units — no ATR feed, no percentage or per-symbol knob to tune. (R is anchored to the lifecycle's *original* risk, so it keeps working after the stop reaches breakeven.) The give-back close remains available for `PROFIT_LOCK_TRAIL_ENABLED=false`.
...
| `PROFIT_LOCK_TRAIL_ARM_R` | `0.5` | Arm the trailing ratchet once peak favorable run reaches this multiple of initial risk (was effectively 1R; most trades peak below 1R so the trail never engaged) |
| `PROFIT_LOCK_TRAIL_LOCK_FRAC` | `0.5` | Once armed, lock at least this fraction of the peak favorable run |
| `PROFIT_LOCK_TRAIL_DISTANCE_R` | `0.75` | ...or trail the stop this many R below the running peak, whichever locks MORE |
