...
2) Risk Management & Position Sizing
   - [ ] Add ATR-based stop/target helpers, risk-per-trade sizing, and portfolio-level limits (daily drawdown, trades per coin/day, kill switch). (Partial: ATR sizing + per-symbol daily cap + USDT-based daily drawdown kill switch + 1h-alignment requirement + timeframe-conflict gate + code-driven profit-lock (breakeven ratchet + give-back cap) + no-chase-after-win guard + regime throttle (confidence/size) + trend-aligned shorts + live-snapshot extremes reset shipped; fill/PnL-based kill switch pending.)
...
## Execution Log
- **Profitability audit: execution integrity, selective opportunity, and inference cost** (2026-07-13): The authoritative retained sample is only weak-positive: **15 closes, +0.243 USDT, 46.7% win rate, PF 1.09**; bootstrap resampling gives only ~52% probability of positive aggregate PnL, and removing the two largest ZEC winners leaves **-1.95 USDT**. ETH is the clearest loser (-1.66 over 3 closes), while ZEC carries the result (+1.96 over 3). Fixed several mechanisms that made this worse or misreported it: (1) performance no longer double-counts a local estimated reduction and KuCoin's later cumulative closed-position PnL (the ZEC +0.5606/+1.4366 duplication); (2) `exitPrice`/`closeType` now survive memory reload, so no-chase and close dedup remain active after restart; false `hold-close-only` outcomes no longer pollute streak/Kelly stats; both loop and agent now share a 90-day learning horizon; (3) futures entries require TP+SL, are shrunk to the stop-defined equity risk budget, and only one pending entry per symbol may exist; bot GTC entries are tagged and actually expire after `ENTRY_LIMIT_EXPIRY_MINUTES`; (4) the unused 24h volatility cap is now enforced in the screener and entry gates, rotating away from extreme movers like LAB before they monopolize dozens of cycles; futures decisions prefer futures candles with spot fallback; (5) the blanket BTC-bearish alt veto now has a narrow, symbol-agnostic relative-strength exception (1D/4H/1H/15M all bullish, strong alignment, confidence >=0.82) at 50% size, addressing the repeated missed ZEC leadership without reopening ordinary high-beta alt risk; (6) model economics were brought under control: logs showed **796 runs / 133.1M input tokens / 5,300 requests** for the six-day file, vastly disproportionate to a ~$72 account. Protection remains every minute, but flat discretionary calls are throttled to hourly and active-book calls to five minutes; max turns reduced to 30 and prompt pressure to force trades/repeat news/account calls removed. **272 tests pass.** Direct public-web refresh was attempted but the environment's network approval service failed; the market read used the bot's same-day KuCoin/CoinDesk research logs (risk-off, BTC bearish daily, leverage flush/geopolitical and inflation-week pressure, with isolated ZEC relative strength).
- **Regime throttle + trend-aligned shorts + extremes-reset fix** (2026-06-09): Reviewed prod logs/memory after the profit-lock deploy (first `Profit-lock enabled` 06-08 15:38; the −3.08 ETH loss at 05:06 predates it). The guards work — give-back closed an ETH winner (16:30, peak +14.1px), breakeven ratchet moved a BTC stop up (21:45) — but the bot is still unprofitable, now bleeding via churn: **10 entries since 06-08, 8 ETH / 2 BTC, 100% longs, 0 shorts**, ~$598 gross notional on a ~$77 account. Root problem = a long-only trap: logs show the agent *tried* to short (`ANTI-FOMO BLOCK: futures limit sell ETH/BTC — daily bearish exhausted`) but the anti-FOMO gate forces it to only long oversold bounces, into a market in Extreme Fear (F&G 8–13, 71% odds ETH→$1,500, hawkish Fed/Iran). Implemented three fixes (new `src/regime.py`, pure + unit-tested): **(A)** `position_extremes` now reset off the **live exchange snapshot** in `trading_loop` (`_live_extremes_map`) instead of memory's trade-ledger reconstruction — fixes the bug where ETH `openTs` stuck at 06-07 across 7 closes (futures triggered-closes aren't written to the ledger, so the phantom long never reset), which fed the agent a stale `peakPnl=12.02`. **(B)** Regime throttle: in a hostile (bearish/RSI-exhausted) daily, raise the confidence floor (`REGIME_CAUTION_MIN_CONFIDENCE=0.75`) and shrink size (`REGIME_CAUTION_SIZE_FACTOR=0.6`) in both futures entry paths. **(D)** Trend-aligned shorts: `allow_trend_aligned_short` re-permits a short past the anti-FOMO gate in an exhausted-bearish daily **only** when 1h+15m both confirm and confidence ≥ `TREND_SHORT_MIN_CONFIDENCE=0.78` (downstream 1h-align/TF-conflict gates then pass cleanly). Config = `RegimeConfig` (env `REGIME_*` / `TREND_*`), all enabled by conservative default since prod `.env` omits them. 139/139 tests pass (12 new in `test_regime.py`).
- **Code-driven profit-lock + no-chase guard** (2026-06-08): Diagnosed the ETH futures round-trip on 2026-06-07/08 — a DCA'd long (avg ~1622) peaked at **+8.54 USDT** unrealized, booked only **+2.42** at TP, then the bot opened a fresh, larger **$85.59 long at the 1711.76 top** (the audit reported "no open positions" right after the TP fired), which round-tripped to a **-3.08** stop-out. Market context (web search): a historic macro crash — hawkish Fed (no 2026 cuts), fresh US–Iran strikes / Strait-of-Hormuz oil spike, Saylor's Strategy selling BTC, and record ETF outflows — drove ETH to a multi-year low ~$1,505 with daily RSI ~12.7 (extreme oversold). The long was riding an oversold dead-cat bounce against a bearish/exhausted daily that the bot itself kept flagging. The directional read was fine; the failure was **risk mechanics left to LLM discretion** — the prompt *suggested* trailing/tightening (agent.py) but nothing enforced it, and `peakPnl` was tracked only for post-hoc analysis. Fix: moved two rules from prompt to code in new `src/protection.py`. **P1 profit-lock** — `ProtectionManager.run(snapshot)` called every poll from `trading_loop` (independent of whether the agent runs): once favorable excursion ≥ `PROFIT_LOCK_BREAKEVEN_TRIGGER_R`×initial-risk, ratchet the SL to fee-adjusted breakeven; if price gives back ≥ `PROFIT_LOCK_GIVEBACK_PCT` of the peak run, market-close (reduce-only `closeOrder`) to lock the gain. Decisions are made in price-space (favorable excursion vs initial risk), avoiding contract-multiplier/PnL-source mismatch; pure `decide_protection()` is unit-tested, including an ETH-incident regression. **P2 no-chase** — `should_block_chase()` wired into the spot + futures entry guards (mirrors the post-loss cooldown): blocks same-direction re-entry at a worse price than a recent winning close within `POST_WIN_COOLDOWN_MINUTES`, which would have blocked the 1711.76 re-long outright while still allowing a genuine pullback re-entry. Needs the exit price, so `MemoryStore.log_decision` now stores `exitPrice`/`closeType` and a new `recent_win_close()` queries it (main.py captures the exit price on each triggered close). Config = `ProfitProtectionConfig` (env `PROFIT_LOCK_*` / `NO_CHASE_*` / `POST_WIN_COOLDOWN_MINUTES`); `PROFIT_LOCK_DRY_RUN=true` logs intended actions without placing orders for safe observation before arming on live funds. Both guards default-enabled and never raise out of the poll loop. Also fixed two pre-existing test fixtures (`test_config`, `test_telegram`) that were already missing the `dashboard` field. 127/127 tests pass (15 new in `test_protection.py`).
